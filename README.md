...
- `examples/` содержит примеры использования компонентов проекта.

## Документация

Базовое руководство для старта: [docs/quickstart.md](docs/quickstart.md)

Полный API всех модулей: [docs/API.md](docs/API.md)

Архитектура системы: [docs/architecture.md](docs/architecture.md)

Часто задаваемые вопросы: [docs/FAQ.md](docs/FAQ.md)
...
Запустить все тесты (unit + integration + benchmark):
...
Запустить benchmark-тесты производительности:
...
Установить pre-commit хуки для локальной проверки перед коммитом:

```bash
make precommit-install
```

ПроверитьSecrets в коде:
